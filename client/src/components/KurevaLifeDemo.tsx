...
function FoodView({ selectedFood, setSelectedFood }: { selectedFood: typeof seasonalFoods[number]; setSelectedFood: React.Dispatch<React.SetStateAction<typeof seasonalFoods[number]>> }) {
  ...
  return <div className="space-y-6"><div><div className="text-[10px] font-mono uppercase tracking-wider text-[#5E806E]">Septiembre · referencia para España</div><h3 className="font-display text-2xl font-bold text-[#0F3A2D] mt-1">Alimentos de temporada, ideas sencillas y sin recetas milagro.</h3><p className="text-sm text-[#5E806E] mt-2 max-w-3xl">Pulsa una fruta o verdura para abrir su ficha justo debajo. Es información general de alimentación; ante cualquier duda sobre tu salud, necesidades o medicación, consulta con tu médico o profesional sanitario.</p></div><div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">{seasonalFoods.map((food) => { const Icon = food.icon; const selected = food.id === selectedFood.id; return <article key={food.id} className={`rounded-2xl border transition-all overflow-hidden ${selected ? "border-[#0F3A2D] ring-2 ring-[#D9FF2B] ring-offset-2" : "border-[#DCD4C4]"} bg-white`}><button onClick={() => setSelectedFood(food)} aria-expanded={selected} aria-controls={`food-details-${food.id}`} className="w-full p-5 text-left hover:bg-[#F5F1E7] focus:outline-hidden focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0F3A2D]"><div className={`w-10 h-10 rounded-xl ${food.color} text-white flex items-center justify-center`}><Icon className="w-5 h-5" /></div><div className="text-[10px] text-[#5E806E] mt-4">{food.type}</div><strong className="font-display block text-[#0F3A2D] mt-1">{food.name}</strong><span className="inline-flex items-center gap-1 text-xs font-bold text-[#0F3A2D] mt-3 underline decoration-[#D9FF2B] decoration-2 underline-offset-4">{selected ? "Ficha abierta" : "Ver ficha"}<ChevronRight className={`w-3.5 h-3.5 transition-transform ${selected ? "rotate-90" : ""}`} /></span></button>{selected && <div id={`food-details-${food.id}`} className="border-t border-[#DCD4C4] bg-[#F5F1E7] px-5 py-4 motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-top-1 duration-200"><p className="text-xs text-[#173A2E] leading-relaxed"><strong>Aporte general:</strong> {food.vitamin}</p><p className="text-xs text-[#5E806E] leading-relaxed mt-2"><strong className="text-[#173A2E]">Idea sencilla:</strong> {food.idea}</p></div>}</article>; })}</div><div className="rounded-3xl bg-[#0F3A2D] p-6 sm:p-8 text-white grid md:grid-cols-[auto_1fr] gap-5 items-start"><div className={`w-14 h-14 rounded-2xl ${selectedFood.color} text-white flex items-center justify-center`}><FoodIcon className="w-7 h-7" /></div><div><div className="text-[#D9FF2B] text-xs font-bold uppercase tracking-wider">Ficha abierta · {selectedFood.type}</div><h4 className="font-display text-2xl font-bold mt-1">{selectedFood.name}</h4><p className="text-white/75 mt-3 text-sm"><strong className="text-white">Aporte general:</strong> {selectedFood.vitamin}</p><p className="text-white/75 mt-2 text-sm"><strong className="text-white">Idea sencilla:</strong> {selectedFood.idea}</p><div className="mt-4 rounded-xl bg-white/10 p-3 text-xs text-white/70"><ShieldCheck className="w-4 h-4 inline-block align-text-bottom text-[#D9FF2B] mr-1.5" /> La app final no sustituirá consejos médicos ni pautas nutricionales personalizadas.</div></div></div><div className="rounded-xl bg-[#FFF7D8] border border-[#E8D592] p-4 text-xs text-[#5A4B13]">En una futura versión, la comunidad podrá compartir ideas y recetas con normas de respeto, moderación y privacidad. Esta demo no publica perfiles, fotos ni recetas reales.</div></div>;
}
