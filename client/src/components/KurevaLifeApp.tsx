...
type AnalyticRecord = { id: string; date: string; label: string; values: { glucose: string; cholesterol: string; hba1c: string }; note: string };
...
const PILOT_REOPENS_AT = "2026-09-21T08:00:00.000Z";
...
export function KurevaLifeApp() {
  ...
  const exportPdf = () => {
    ...
    heading("Comparador de analíticas");
    if (!analytics.length) item("No se añadieron analíticas comparables.");
    analytics.slice(0, 3).sort((a, b) => a.date.localeCompare(b.date)).forEach((analytic) => item(`${analytic.label} · ${analytic.date} · glucosa: ${analytic.values.glucose || "sin anotar"} · colesterol: ${analytic.values.cholesterol || "sin anotar"} · HbA1c: ${analytic.values.hba1c || "sin anotar"}${analytic.note ? ` · ${analytic.note}` : ""}`));
...
function HealthPanel({ profile, setProfile, records, addRecord, documents, setDocuments, analytics, setAnalytics }: { profile: Profile; setProfile: React.Dispatch<React.SetStateAction<Profile>>; records: HealthRecord[]; addRecord: (record: Omit<HealthRecord, "id" | "createdAt">) => void; documents: DocumentItem[]; setDocuments: React.Dispatch<React.SetStateAction<DocumentItem[]>>; analytics: AnalyticRecord[]; setAnalytics: React.Dispatch<React.SetStateAction<AnalyticRecord[]>> }) {
  ...
  const [analyticLabel, setAnalyticLabel] = useState("Analítica general");
  const [analyticGlucose, setAnalyticGlucose] = useState("");
  const [analyticCholesterol, setAnalyticCholesterol] = useState("");
  const [analyticHba1c, setAnalyticHba1c] = useState("");
  const [analyticNote, setAnalyticNote] = useState("");
...
function CommunityPanel() {
  ...
  const closesAt = new Date(PILOT_CLOSES_AT).toISOString();
