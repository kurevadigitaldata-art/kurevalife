...
export function KurevaLifeApp() {
  ...
  const exportPdf = () => {
    ...
    const text = (copy: string, x: number, y: number, size = 10, color: [number, number, number] = ink, style: "normal" | "bold" = "normal", options?: { align?: "left" | "center" | "right" }) => { doc.setFont("helvetica", style); doc.setFontSize(size); doc.setTextColor(...color); doc.text(copy, x, y, options); };
    const body = (copy: string, x: number, y: number, maxWidth: number, color: [number, number, number] = moss) => { doc.setFont("helvetica", "normal"); doc.setFontSize(9); doc.setTextColor(...color); const lines = doc.splitTextToSize(copy, maxWidth) as string[]; doc.text(lines, x, y, { lineHeightFactor: 1.45 }); return y + lines.length * 4.6; };
    const addFooter = () => { doc.setDrawColor(220, 212, 196); doc.line(18, height - 18, width - 18, height - 18); text("KurevaLife · resumen local de prueba", 18, height - 11, 7.3, moss); text(new Intl.DateTimeFormat("es-ES", { day: "2-digit", month: "long", year: "numeric" }).format(new Date()), width - 18, height - 11, 7.3, moss, "normal", { align: "right" }); };
    const newPage = () => { addFooter(); doc.addPage(); doc.setFillColor(...cream); doc.rect(0, 0, width, height, "F"); doc.setFillColor(...forest); doc.rect(0, 0, width, 22, "F"); text("KUREVALIFE · RESUMEN DE PRUEBA", 18, 14, 8, cream, "bold"); return 38; };
    ...
    text("K", width - 22, 22.2, 11, forest, "bold", { align: "center" });
    text("KUREVALIFE", 18, 16, 11, cream, "bold");
    text("BY KUREVA · RESUMEN DE PRUEBA", 18, 24, 7.2, lime, "bold");
    doc.setTextColor(15, 58, 45, 0.06); doc.setFont("helvetica", "bold"); doc.setFontSize(102); doc.text("K", width - 43, 116);
    text("Mi recorrido de prueba", 18, 64, 23, forest, "bold");
    let y = body("Este resumen recoge únicamente las interacciones que has hecho en esta sesión de simulación. No es un historial clínico ni contiene una interpretación médica.", 18, 78, 172);
    y += 8;
    doc.setFillColor(220, 232, 221); doc.roundedRect(18, y, 172, 18, 4, 4, "F");
    text("Observación importante", 23, y + 6, 8, forest, "bold");
    body("No somos médicos ni sustituimos el trabajo de tu médico. KurevaLife solo organiza los registros que tú elijas para ayudarte a no olvidar nada importante antes, durante o después de una cita.", 23, y + 11, 160, forest);
    y += 25;

    const requireSpace = (needed: number) => { if (y + needed > height - 26) y = newPage(); };
    requireSpace(38);
    text("Bloques explorados", 18, y, 13, forest, "bold"); y += 9;
    blocks.forEach((block) => { requireSpace(7); text(`${completed.includes(block.id) ? "✓" : "○"} ${block.time} · ${block.title}`, 20, y, 9, ink, "bold"); y += 6; });
    y += 5;
    requireSpace(28);
    text("Alarmas de prueba", 18, y, 13, forest, "bold"); y += 9;
    if (alarms.length === 0) { text("No se registraron alarmas durante esta prueba.", 20, y, 9, moss); y += 6; } else { alarms.forEach((alarm) => { requireSpace(7); text(`• ${alarm.title}`, 20, y, 9, ink); y += 6; }); }
    y += 5;
    requireSpace(28);
    text("Registros realizados", 18, y, 13, forest, "bold"); y += 9;
    if (records.length === 0) { text("No se registraron mediciones o molestias durante esta prueba.", 20, y, 9, moss); y += 6; } else { records.forEach((record) => { requireSpace(7); text(`• ${record.title} · ${record.createdAt}`, 20, y, 9, ink); y += 6; }); }
    y += 5;
    requireSpace(28);
    text("Ficha de prueba", 18, y, 13, forest, "bold"); y += 8;
    y = body(`Edad: ${healthProfile.age || "Sin registrar"} · Peso: ${healthProfile.weight ? `${healthProfile.weight} kg` : "Sin registrar"} · Diagnósticos anotados: ${healthProfile.diagnoses.length ? healthProfile.diagnoses.join(", ") : "Ninguno"}.`, 20, y, 168) + 5;
    requireSpace(30);
    text("Documentos analíticos de prueba", 18, y, 13, forest, "bold"); y += 9;
    if (analyticDocuments.length === 0) { text("No se añadieron documentos analíticos en esta prueba.", 20, y, 9, moss); y += 6; } else { analyticDocuments.forEach((document) => { requireSpace(7); text(`• ${document.date || "Sin fecha"} · ${document.title}`, 20, y, 9, ink); y += 6; }); }
    y += 5;
    const noteLines = doc.splitTextToSize(note || "Sin nota.", 168) as string[];
    requireSpace(11 + noteLines.length * 4.6);
...
function WelcomeScreen({ step, setStep, onStart }: { step: number; setStep: React.Dispatch<React.SetStateAction<number>>; onStart: () => void }) {
  const pages = [
    { eyebrow: "Bienvenida anticipada", title: "Gracias por acercarte a KurevaLife.", copy: "Estás entrando en una prueba guiada de una app que nace para acompañar lo cotidiano: recordar, reunir preguntas, ordenar citas y tener a mano lo que tú decides registrar.", note: "KurevaLife no es una aplicación médica, no diagnostica y no sustituye el criterio ni el trabajo de tu médico." },
    { eyebrow: "Cinco minutos, una experiencia completa", title: "Pruébala como si fuera tu app.", copy: "Podrás explorar tu día, crear una alarma visual, registrar ejemplos de tensión, glucosa o molestias, añadir una ficha de salud local, organizar analíticas y abrir alimentos de temporada. Nada de lo que escribas en la demo se guarda de forma permanente.", note: "El temporizador solo propone un recorrido. Cuando llegue a cero puedes continuar, parar o repetir la prueba con calma." },
    { eyebrow: "Kureva crece contigo", title: "Tu opinión guía el siguiente paso.", copy: "Al terminar puedes dejar una valoración privada, participar voluntariamente en la conversación temporal de testers o apuntarte a las novedades. Cada sugerencia nos ayuda a hacer una experiencia más clara, accesible y humana.", note: "En el grupo temporal no compartas diagnósticos, medicación, analíticas, teléfonos, correos ni otra información personal." },
  ];
  const page = pages[step];
  return <section className="min-h-screen bg-[#F5F1E7] flex items-center py-8"><div className="container max-w-5xl"><a href="/" className="inline-flex items-center gap-2 text-sm font-bold text-[#0F3A2D] mb-5"><span className="w-8 h-8 rounded-lg bg-[#0F3A2D] text-[#D9FF2B] flex items-center justify-center font-display">K</span> Kureva <span className="font-normal text-[#5E806E]">· volver a la web</span></a><div className="rounded-3xl overflow-hidden shadow-2xl border border-[#164D3C] bg-[#0F3A2D] text-white grid lg:grid-cols-[1.1fr_0.9fr]"><div className="p-7 sm:p-11 space-y-7"><div className="inline-flex items-center gap-2 text-[#D9FF2B] text-xs font-bold uppercase tracking-wider"><Sparkles className="w-4 h-4" /> {page.eyebrow}</div><div className="w-55 max-w-full"><img src="/manus-storage/kurevalife-lockup_f908d233.svg" alt="KurevaLife by Kureva" className="w-full brightness-0 invert" /></div><h1 className="font-display text-4xl sm:text-5xl font-bold leading-tight">{page.title}</h1><p className="text-white/75 text-lg leading-relaxed max-w-xl">{page.copy}</p><div className="rounded-xl bg-white/10 border border-white/10 p-4 text-sm text-white/75 leading-relaxed"><ShieldCheck className="w-4 h-4 inline-block align-text-bottom text-[#D9FF2B] mr-1.5" /> {page.note}</div><div className="flex gap-3 flex-wrap">{step < pages.length - 1 ? <button onClick={() => setStep((value) => value + 1)} className="kureva-btn-accent">Siguiente <ChevronRight className="w-4 h-4" /></button> : <button onClick={onStart} className="kureva-btn-accent">Comenzar prueba de 5 minutos <ChevronRight className="w-4 h-4" /></button>}{step > 0 && <button onClick={() => setStep((value) => value - 1)} className="px-4 py-3 font-semibold text-sm text-white/80 hover:text-white">Anterior</button>}</div><a href="/#metodo" className="inline-flex items-center text-sm font-bold text-white/80 underline decoration-[#D9FF2B] decoration-2 underline-offset-5 hover:text-white">¿Quieres conocer Kureva y todo lo que está por venir?</a></div><aside className="bg-[#09241B] p-7 sm:p-11 flex flex-col justify-between gap-12"><div><div className="relative w-32 h-32"><div className="absolute inset-0 rounded-full border border-[#D9FF2B]/35 kureva-k-orbit" /><div className="absolute inset-5 rounded-full border border-white/10 kureva-k-orbit-reverse" /><div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-13 h-13 rounded-2xl bg-[#D9FF2B] text-[#0F3A2D] flex items-center justify-center font-display text-2xl font-bold shadow-[0_0_32px_rgba(217,255,43,.28)] kureva-k-breathe">K</div><span className="absolute left-0 top-1/2 w-2.5 h-2.5 rounded-full bg-[#D9FF2B]" /><span className="absolute right-2 top-3 w-2 h-2 rounded-full bg-white/70" /></div><h2 className="font-display text-2xl font-bold mt-7">Una prueba a tu ritmo.</h2><p className="text-sm text-white/65 mt-2">Puedes ampliar el texto, subir el contraste y navegar con teclado. No hay rachas, puntos ni penalizaciones.</p></div><div className="space-y-2">{pages.map((item, index) => <button key={item.eyebrow} onClick={() => setStep(index)} aria-current={step === index ? "step" : undefined} className={`w-full text-left rounded-xl px-3 py-3 flex items-center gap-3 ${step === index ? "bg-white/10" : "hover:bg-white/5"}`}><span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${index <= step ? "bg-[#D9FF2B] text-[#0F3A2D]" : "bg-white/10 text-white/65"}`}>{index < step ? <Check className="w-3.5 h-3.5" /> : index + 1}</span><span className={`text-xs ${step === index ? "text-white" : "text-white/60"}`}>{item.eyebrow}</span></button>)}</div></aside></div></div></section>;
}
